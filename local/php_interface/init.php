    ...
    */
    ...
    function customizeNewOrderMail($orderID, &$eventName, &$arFields){
        ...
        if ($orderArr['PAY_SYSTEM_ID'] == 12) { //если оплата по безналу юриком
            ...
            $arFields['PAYMENT_LINK'] = "Менеджер отправит счет на оплату в рабочее время.<br />Внимание! Заказы, оплаченные после 25.12.2016, будут отправлены в начале следующего года.";
        } else {
            $arFields['EMAIL_PAY_SYSTEM'] = getOrderPaySystemName($orderArr['PAY_SYSTEM_ID']);
        }
        ...
        if ($orderArr["PAY_SYSTEM_ID"] == 13 || $orderArr["PAY_SYSTEM_ID"] == 14) {
            ...
            $arFields["PAYMENT_LINK"] = "Для оплаты заказа перейдите по <a href='https://www.alpinabook.ru/personal/order/payment/?ORDER_ID=".$orderArr["ID"]."'>ссылке</a>.<br />Внимание! Заказы, оплаченные после 25.12.2016, будут отправлены в начале следующего года.";
        }
