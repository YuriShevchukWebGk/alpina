...
if ($CAT_RIGHT>="R"):
	...
	include_once($GLOBALS["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/include.php");
	...
	if($CAT_RIGHT>="W" && check_bitrix_sessid())
	{
		...
		if($REQUEST_METHOD=="POST" && strlen($Update)>0)
		{
			...
			else
			{
				...
				if(time() > $iTimestamp || !file_exists($_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/cities.csv"))
				{
			...
			<td colspan = "2" align="center">
				...
				foreach($personType as $val)
				{
					...
					<table class = "internal" width = "80%">
						...
						require($_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/row.php");?>
