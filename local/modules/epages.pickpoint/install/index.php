...
Class epages_pickpoint extends CModule
{
	...
	function DoUninstall()
	{
		...
		if($step<2)
		{
			...
			$APPLICATION->IncludeAdminFile(
				...
				$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/unstep1.php"
			);
		}
		elseif($step==2)
		{
			...
			$APPLICATION->IncludeAdminFile(
				GetMessage("ST_INSTALL_TITLE"),
				$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/unstep2.php"
			);
		}
	...
	function InstallDB()
	{
		...
		$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/db/".$DBType."/install.sql");
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/db/".$DBType."/uninstall.sql");
			if($this->errors !== false)
			{
				$APPLICATION->ThrowException(implode("", $this->errors));
				return false;
	...
	{
		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/admin",
			$_SERVER["DOCUMENT_ROOT"]."/bitrix/admin",
			true
		);
		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/images/",
			$_SERVER["DOCUMENT_ROOT"]."/bitrix/images/epages.pickpoint/",
			true,
			true
		);
		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/themes/",
			$_SERVER["DOCUMENT_ROOT"]."/bitrix/themes/",
			true,
			true
		);
		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/js/",
			$_SERVER["DOCUMENT_ROOT"]."/bitrix/js/",
			true,
			true
		);
		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/delivery/",
			$_SERVER["DOCUMENT_ROOT"]."/bitrix/php_interface/include/sale_delivery/",
			true,
			true
		);
		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/payment/",
	...
	function UnInstallFiles()
	{
		...
		DeleteDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/install/themes/.default/",
