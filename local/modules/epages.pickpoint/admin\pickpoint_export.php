...
require_once($_SERVER["DOCUMENT_ROOT"]."/local/modules/epages.pickpoint/include.php");
...
if(!empty($_REQUEST["EXPORT"])&&($_REQUEST["export"]))
{
    ...
    {
        /*$order_info = CSaleOrder::GetByID($iOrderID);
        while ($order = $order_info -> Fetch()) {
            $order_person_type = $order["PERSON_TYPE_ID"];
        }
        if ($order_person_type == 1) {
            $order_props = CSaleOrderPropsValue::GetOrderProps($iOrderID);
            while ($order_props_arr = $order_props -> Fetch()) {
                if ($order_props_arr["ORDER_PROPS_ID"] == "ADDRESS") {
                    $address = $order_props_arr["VALUE"];
                }
            } 
        } else if ($order_person_type == 2) {
            $order_props = CSaleOrderPropsValue::GetOrderProps($iOrderID);
            while ($order_props_arr = $order_props -> Fetch()) {
                if ($order_props_arr["ORDER_PROPS_ID"] == "F_ADDRESS") {
                    $address = $order_props_arr["VALUE"];
                }
            }
        }*/
...
<script>
    ...
    function CheckFields_PP_Export(cSelectAll)
    {
        Table = document.getElementById("table_export");
        arInputs = Table.getElementsByClassName("cToExport");
        for(i=0;i<arInputs.length;i++)
...
<?/*                        <td>
        ...
        <?$tabControl->Buttons();?>
        <input type="submit" class="adm-btn-save" name="export" onclick = "return CheckFields_PP_Export();" value="<?echo GetMessage("PP_EXPORT")?>">
