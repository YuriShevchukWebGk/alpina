...
if (isset($arCurrentValues['IBLOCK_ID']) && 0 < intval($arCurrentValues['IBLOCK_ID']))
{
	...
	if($arIblock = $dbIblock->Fetch())	{
	    $seriesIblockID = intval($arIblock["ID"]);
		$arSelectSeries = Array("ID", "NAME");//IBLOCK_ID и ID обязательно должны быть указаны, см. описание arSelectFields выше
		$arFilterSeries = Array("IBLOCK_ID"=>IntVal($seriesIblockID), "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
		$dbSeries = CIBlockElement::GetList(Array(), $arFilterSeries, false, Array(), $arSelectSeries);
