...
foreach ($arResult["ITEMS"] as $key => $arItem) {
    ...
    $gift_book_addings = CIBlockElement::GetList (array(), array("IBLOCK_ID" => SUSPENDED_BOOKS_BUYERS_IBLOCK, "PROPERTY_GIFT_BOOK" => $arItem["ID"]), false, false, array("NAME", "ID", "PROPERTY_GIFT_BOOK", "PROPERTY_GIFT_QUANTITY"));
    $count = 0;
    while ($addings = $gift_book_addings -> Fetch()) {
        $count++;
        $arResult[$arItem["ID"]]["BUYERS"][]["NAME"] = $addings["NAME"];
        $arResult[$arItem["ID"]]["BUYERS"][]["QUANTITY"] = $addings["PROPERTY_GIFT_QUANTITY_VALUE"];    
    }
    $arResult[$arItem["ID"]]["GIFTS_COUNT"] = $count;
