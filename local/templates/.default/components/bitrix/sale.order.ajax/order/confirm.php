            ...
            <? if ($arResult["PAY_SYSTEM"]["ID"] != 1 && $arResult["PAY_SYSTEM"]["ID"] != 12) { ?>
                ...
                if ($arResult["PAY_SYSTEM"]["ID"] == CASHLESS_PAYSYSTEM_ID && $arResult["ORDER"]["PERSON_TYPE_ID"] == LEGAL_ENTITY_PERSON_TYPE_ID) {
                    ...
                    <div class="expense_offer_block">
                        ...
                        <br>
                        <div class="order_doc_title"><?= GetMessage("BLANK_NUMBER_TITLE") ?> <?= $arResult["ORDER"]["ID"] ?> <?= GetMessage("FROM_DATE") ?> <?= FormatDate("j F", MakeTimeStamp(date("d.m.Y", strtotime($order_info["DATE_INSERT"])), "DD.MM.YYYY HH:MI:SS")) . " " . date("Y", strtotime($order_info["DATE_INSERT"])) . GetMessage("YEAR") ?></div>
                        <br>
