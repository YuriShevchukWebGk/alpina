...
if ($arResult['MODULES']['currency'])
{
	...
	// ÐŸÐ¾Ð»ÑƒÑ‡Ð°ÐµÐ¼ Ð¿ÐµÑ€ÐµÐ²Ð¾Ð´Ñ‡Ð¸ÐºÐ°
	$arResult["TRANSLATOR"] = '';
    ...
    // Ð¿Ð¾Ð»ÑƒÑ‡ÐµÐ½Ð¸Ðµ Ð¸Ð¼Ñ‘Ð½ Ð°Ð²Ñ‚Ð¾Ñ€Ð¾Ð² ÐºÐ½Ð¸Ð³Ð¸
   ...
   foreach ($arOrder["BASKET_ITEMS"] as $arOneItem) {
	...
	//ÐŸÐ¾Ñ€Ð¾Ð³ Ð´Ð»Ñ Ð±ÐµÑÐ¿Ð»Ð°Ñ‚Ð½Ð¾Ð¹ Ð´Ð¾ÑÑ‚Ð°Ð²ÐºÐ¸
    ...
    while ($date_close = $object_date_close->Fetch()) {
        if($date_close["CODE"] == "close"){
            $arResult["CLOSE_DATE"] = explode(', ', $date_close["NAME"]);     // âûòàñêèâàåò çàêðûòûå äàòû äîñòàâêè
        } else {
            $arResult["OPEN_DATE"] = explode(', ', $date_close["NAME"]);   // âûòàñêèâàåò îòêðûòûå äàòû äîñòàâêè
        }
    }
?>