...
<?
	...
	if ($psum < 2000) {
		$printDiscountText = "<span class='sale_price'><span class='addlink'>Добавьте товаров</span> на " . round((2000 - $psum), 2) ." руб. и получите БЕСПЛАТНУЮ доставку";
	} elseif ($psum < 3000 && CCatalogDiscountSave::GetDiscount(array('USER_ID' => $USER->GetID()))[0][VALUE] == 10) {
		$printDiscountText = "<span class='sale_price'><span class='addlink'>Добавьте товаров</span> на " . round((3000 - $psum), 2)." руб. и получите скидку 19%";

	} elseif ($psum < 3000 && CCatalogDiscountSave::GetDiscount(array('USER_ID' => $USER->GetID()))[0][VALUE] == 20) {
		$printDiscountText = "<span class='sale_price'><span class='addlink'>Добавьте товаров</span> на " . round((3000 - $psum), 2)." руб. и получите скидку 28%";
