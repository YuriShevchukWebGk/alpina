                        ...
                        ****************/
                        ...
                        if ($arItem["PARAM2"] == AUTHORS_IBLOCK_ID) { 
                            ...
                            <div class="searchBook">
                                <div>
                                    <a href="<?= $arItem["CANONICAL_PAGE_URL"] ?>">
                                        <div class="search_item_img">
                                            <?
                                            $pict["src"] = $arResult["AUTHOR_INFO"][$arItem["ID"]]["PICTURE"]["src"]
                                                ? $arResult["AUTHOR_INFO"][$arItem["ID"]]["PICTURE"]["src"]
                                                : "/images/no_photo.png";
                                            $pict["width"] = $arResult["AUTHOR_INFO"][$arItem["ID"]]["PICTURE"]["src"]
                                                ? ""
                                                : "155";
                                            ?>
                                ...
                                <div class="descrWrap">
                                    <a href="<?= $arItem["URL"] ?>">
                                        ...
                                        <div class="description">
                                            <?=  $arResult["AUTHOR_INFO"][$arItem["ID"]]["PROPERTY_AUTHOR_DESCRIPTION_VALUE"]["TEXT"] ?>
                                        </div>
                                    </a>
                                </div>
                            </div>
                        ...
                        if ($arItem["SORT"]) {
                            ...
                            <?if ($USER->IsAuthorized() && ($arResult["SAVINGS_DISCOUNT"][0]["SUMM"] > 0 || $arResult["CART_SUM"] > 0)) {// blackfriday черная пятница
                                    if ($arResult[$arItem["ID"]]["ITEM_WITHOUT_DISCOUNT"] == 'Y' || ($arResult["SAVINGS_DISCOUNT"][0]["SUMM"] < $arResult["SALE_NOTE"][0]["RANGE_FROM"])) {
                                        $discount = 0;
                            ...
                            if ($discount) {
                                $newPrice = round (($arResult["BOOK_INFO"][$arItem["ID"]]["CATALOG_PRICE_1"]) * (1 - $discount / 100), 2);
                                if (strlen (stristr($newPrice, ".")) == 2) {
                                    $newPrice .= "0";
                                }
                            } else {
                                $newPrice = $arResult["BOOK_INFO"][$arItem["ID"]]["CATALOG_PRICE_1"];
                            }
                            if (!empty($arResult["BOOK_INFO"][$arItem["ID"]]["DISCOUNT_INFO"])) {
                                $item_discount_value = ceil($arResult["BOOK_INFO"][$arItem["ID"]]["DISCOUNT_INFO"]["VALUE"]);
                            }
                            if ($item_discount_value > 0) {
								...
								$newPrice = round ($arResult["BOOK_INFO"][$arItem["ID"]]["CATALOG_PRICE_1"] * (1 - $item_discount_value/100), 2);
								if (strlen (stristr($newPrice, ".")) == 2) {
									$newPrice .= "0";
								}
                            }
                            if($arResult["BOOK_INFO"][$arItem["ID"]]["IBLOCK_SECTION_ID"] == CERTIFICATE_SECTION_ID) {
                                $newPrice = $arResult["BOOK_INFO"][$arItem["ID"]]["CATALOG_PRICE_1"];
                            }
                            ...
                            <div class="searchBook" itemprop="itemListElement" itemscope itemtype="http://schema.org/Book">
								<?$gdeslon .= $arItem["ID"].':'.ceil( $newPrice).',';?>
                                <div>
                                    <a href="<?= $arItem["DETAIL_PAGE_URL"]?>">
                                        <div class="search_item_img">
                                            <?if ($arResult["BOOK_INFO"][$arItem["ID"]]["PICTURE"]["src"]) {?>
                                                <img src="<?=$arResult["BOOK_INFO"][$arItem["ID"]]["PICTURE"]["src"]?>" itemprop="image">
                                            <?} else {?>
                                ...
                                <div class="descrWrap">
                                    <a href="<?= $arItem["URL"] ?>" itemprop="url">
                                        ...
                                        <p class="autorName" itemprop="author"><?= $arResult["BOOK_AUTHOR_INFO"][$arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_AUTHORS_VALUE"]]?></p>
                                        <p class="wrapperType"><?= $arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_COVER_TYPE_VALUE"]?></p>
                                        <?if (($arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_STATE_ENUM_ID"] != getXMLIDByCode(CATALOG_IBLOCK_ID, "STATE", "soon"))
                                            && ($arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_STATE_ENUM_ID"] != getXMLIDByCode(CATALOG_IBLOCK_ID, "STATE", "net_v_nal"))) {
                                        ?>
                                        ...
                                        <?} else if ($arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_STATE_ENUM_ID"] == getXMLIDByCode(CATALOG_IBLOCK_ID, "STATE", "soon")) {?>
                                                <p class="price">Ожидаемая дата выхода: <?= strtolower(FormatDate(
                                                    "j F",
                                                    MakeTimeStamp(
                                                        $arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_SOON_DATE_TIME_VALUE"],
                                                        "DD.MM.YYYY"
                                                    )
                                                )); ?>
                                                </p>
                                        <?} else {?>
                                                <p class="price" style="color:red"><?= $arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_STATE_VALUE"] ?></p>
                                        <?}?>
                                        <div class="description" itemprop="description"><?= $arResult["BOOK_INFO"][$arItem["ID"]]["PREVIEW_TEXT"]?></div>
                                        <?
                                        if ($arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_STATE_ENUM_ID"] != getXMLIDByCode(CATALOG_IBLOCK_ID, "STATE", "net_v_nal")) {
                                                if (($arResult["BASKET_ITEMS"][$arItem["ID"]]["QUANTITY"] == 0) && ($arResult["BOOK_INFO"][$arItem["ID"]]["IBLOCK_SECTION_ID"] != CERTIFICATE_SECTION_ID)) {
                                                    $curr_sect_ID = $arResult["BOOK_INFO"][$arItem["ID"]]["IBLOCK_SECTION_ID"];?>
                                                    <a class="product<?= $arItem["ID"]; ?>"
                                                        href="<?= '/search/index.php?action=ADD2BASKET&id=' . $arItem["ID"] ?>"
                                                        onclick="addtocart(<?= $arItem["ID"]; ?>, '<?= $arItem["TITLE"];?>', '<?= $arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_STATE_ENUM_ID"];?>'); addToCartTracking(<?= $arItem["ID"]; ?>, '<?= $arItem["TITLE"]; ?>', '<?= ceil( $arResult["BOOK_INFO"][$arItem["ID"]]["CATALOG_PRICE_1"]) ?>', '<?echo $arResult["BOOK_INFO"]["SECTIONS"][$curr_sect_ID]["SECTION_INFO"]['NAME'];?>', '1');return false;">
                                                        <?if(intval($arResult["BOOK_INFO"][$arItem["ID"]]["PROPERTY_STATE_ENUM_ID"]) != getXMLIDByCode (CATALOG_IBLOCK_ID, "STATE", "soon")) {?>
                                                            <p class="basket"><?=GetMessage("ADD_IN_BASKET")?></p>
                                                        <?} else {?>
                                                            <p class="basket"><?=GetMessage("ADD_TO_PREORDER")?></p>
                                                        <?}?>
                                                    </a>
                                                <?} elseif($arResult["BOOK_INFO"][$arItem["ID"]]["IBLOCK_SECTION_ID"] == CERTIFICATE_SECTION_ID) {?>
                                                    <a class="product<?= $arItem["ID"]; ?>" href="<?=$arItem['URL'];?>">
                            ...
                            <div class="searchBook">
                                ...
                                <div class="descrWrap">
                                    <a href="<?= $arItem["URL"] ?>">
                                        ...
                                        <div class="description"><?= $arResult["SERIE_INFO"][$arItem["ID"]]["PREVIEW_TEXT"] ?></div>
                                    </a>
                                </div>
                            </div>
