    ...
    <div class="personal_info_wrapper">
        <div class="bx_profile bx_<?= $arResult["THEME"] ?>">
            <form method="post" name="form1" action="<?= $arResult["FORM_TARGET"] ?>?" enctype="multipart/form-data">
                ...
                <input type="text" name="NAME" maxlength="50" value="<?= $arResult["arUser"]["NAME"] ?>" placeholder="<?= GetMessage('NAME') ?>"/><br><br>

                <input type="text" name="LAST_NAME" maxlength="50" value="<?= $arResult["arUser"]["LAST_NAME"] ?>" placeholder="<?= GetMessage('LAST_NAME') ?>"/><br><br>
                ...
                <?if($arResult["USER_PROPERTIES"]["SHOW"] == "Y") {?>
                    ...
                    <?foreach ($arResult["USER_PROPERTIES"]["DATA"] as $FIELD_NAME => $arUserField) {?>
                        <strong><?= $arUserField["EDIT_FORM_LABEL"] ?><?if ($arUserField["MANDATORY"] == "Y") {?><span class="starrequired">*</span><?}?></strong><br/>
