...
if ($_GET['ids']) {
    ...
    foreach ($finalBooks as $books) {
        if($books['cover'] == "Покетбук" || $books['cover'] == "Кожаный переплет" || $books['cover'] == "Переплёт из эко-кожи"){
           $book_cover = '<b>'.$books['cover'].'</b>'; 
        }  else {
        ...
        if ($books['quantity'] > 1)
            ...
            $table .= '<tr>'; 
        $table .= '<td width=500>'.$books['name'].'<br />'.$book_cover.'</td>';
