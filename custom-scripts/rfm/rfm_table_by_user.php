...
(
	...
	//">UF_ORDERS_COUNT"		=>	1,
    //"LOGIN"             	=> "am@alpinabook.ru",
	"ID"					=> 15
	
);

if (!empty($_REQUEST["UF_ORDERS_COUNT"])) {
	$filter[">UF_ORDERS_COUNT"] = $_REQUEST["UF_ORDERS_COUNT"];
}
if (!empty($_REQUEST["UF_ORDERS_SUM_MIN"])) {
	$filter[">UF_ORDERS_SUM"] = $_REQUEST["UF_ORDERS_SUM_MIN"];
}
if (!empty($_REQUEST["UF_ORDERS_SUM_MAX"])) {
	$filter["<UF_ORDERS_SUM"] = $_REQUEST["UF_ORDERS_SUM_MAX"];
}
if (!empty($_REQUEST["UF_ORDERS_COUNT_MIN"])) {
	$filter[">UF_ORDERS_COUNT"] = $_REQUEST["UF_ORDERS_COUNT_MIN"];
}
if (!empty($_REQUEST["UF_ORDERS_COUNT_MAX"])) {
	$filter["<UF_ORDERS_COUNT"] = $_REQUEST["UF_ORDERS_COUNT_MAX"];
}
if (!empty($_REQUEST["UF_LAST_NOW"])) {
	$filter[">UF_LAST_NOW"] = $_REQUEST["UF_LAST_NOW"];
}
if (!empty($_REQUEST["UF_PERSON_TYPE_ID"])) {
	$filter["UF_PERSON_TYPE_ID"] = "2";
}
