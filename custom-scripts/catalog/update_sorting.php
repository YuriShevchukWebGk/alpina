	...
	foreach ($categories as $category) {
		...
		if ($ob = $res->GetNext()) {
			if ($ob['DEPTH_LEVEL'] == 2) {
				...
				echo $ob['NAME'].' '.$ob['ID'].' '.$category[1].'<br />';
			} else {
			   ...
			   while ($arSect = $rsSect->GetNext()) {
				   ...
				   $parents[$ob['ID']]['SORT'] += ($fix - $arSect['SORT']);
			   }
			   $parents[$ob['ID']]['SORT'] += $category[1];
			}
		} else {
			echo '<b>'.$category[0].' '.$category[1].'</b><br />';
		}
	}
	
	foreach ($parents as $parent) {
		$sort = round($fix - ($parent['SORT']/10));
		$bs = new CIBlockSection;
		$arFields = Array(
			"SORT" => $sort,
		);
		$res = $bs->Update($parent['ID'], $arFields);
		echo $parent['NAME'].' '.$parent['ID'].' '.$parent['SORT'].'<br />';
	}
} else {
	echo "ошибка";
}
