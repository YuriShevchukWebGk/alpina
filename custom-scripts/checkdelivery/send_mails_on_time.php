...
if ($USER->isAdmin()) {
	...
	*************/
	...
	while (($NewItemsList = $NewItems -> Fetch()) && ($i < 3))
	{
		...
		<a href="http://www.alpinabook.ru/catalog/'.$curr_sect["CODE"].'/'.$NewItemsList["ID"].'/utm_source=autotrigger&amp;rr_setemail=#EMAIL#&amp;utm_medium=email&amp;utm_term=newbooks&amp;utm_campaign=forgottenMails" target="_blank">
		...
		<a href="http://www.alpinabook.ru/catalog/'.$curr_sect["CODE"].'/'.$NewItemsList["ID"].'/utm_source=autotrigger&amp;rr_setemail=#EMAIL#&amp;utm_medium=email&amp;utm_term=newbooks&amp;utm_campaign=forgottenMails" target="_blank">Подробнее о книге</a>
		</td>
		</tr>
		<tr>
		...
		<a href="http://www.alpinabook.ru/catalog/'.$curr_sect["CODE"].'/'.$NewItemsList["ID"].'/utm_source=autotrigger&amp;rr_setemail=#EMAIL#&amp;utm_medium=email&amp;utm_term=newbooks&amp;utm_campaign=forgottenMails" target="_blank">Купить</a>
		</td>
		</tr>
		</tbody>
		</table>';
	...
	*************/
	...
	while ($arSales = $rsSales->Fetch())
	{
		$id = $arSales["ID"];
		if (
			(time() - strtotime($arSales[DATE_STATUS]))/86400 > 4 && 	// Если прошло больше 4 дней
			(time() - strtotime($arSales[DATE_STATUS]))/86400 < 6 && 	// и меньше 6 дней
			$arSales["EMP_STATUS_ID"] != $userID1) 						// еще не отправляли первое уведомление о собранном заказе
		{
			...
			$notification = "Ваши книги скучают и ждут Вас. Скорее приезжайте за ними, срок хранения вашего заказа истекает уже через три дня.<br />
			...
			(time() - strtotime($arSales[DATE_STATUS]))/86400 >= 6 && 	// Если прошло больше 5 дней
			$arSales["EMP_STATUS_ID"] != $userID2)						// и еще не отправляли второе уведомление
		{
			$subject = 'Истекает срок хранения заказа №'.$id.'. Альпина Паблишер';
			$notification = "Ваши книги скучают и ждут вас. Скорее приезжайте за ними, срок хранения вашего заказа истекает уже завтра.<br />
			Вы можете забрать заказ ".$id." по адресу: метро «Полежаевская», 4-я Магистральная улица, дом 5, подъезд 2, второй этаж.<br /><br />
			Да, кстати, у нас есть несколько хороших новинок, которые должны вам понравиться.";
			...
			for ($i = 1; $i <= $countUsers; $i++) {
				try {
					...
					*/
						...
						$message = new Message();
                        if($_SESSION["MESSAGE_STATE"] != $userIDarrived || $_SESSION["MESSAGE_ORDER"] != $id || $_SESSION["MESSAGE_TRACING"] != $trackingNumber){
						    $result = $message->sendMessage($id,'PD','','',$trackingNumber);
                        }

                        $_SESSION["MESSAGE_STATE"] = $userIDarrived;
					...
					*/
                        ...
                        if($_SESSION["MESSAGE_STATE"] != $userIDreturn || $_SESSION["MESSAGE_ORDER"] != $id || $_SESSION["MESSAGE_TRACING"] != $trackingNumber){
						    $result = $message->sendMessage($id,'PA');
                        }
                        $_SESSION["MESSAGE_STATE"] = $userIDreturn;
                        $_SESSION["MESSAGE_TRACING"] = $trackingNumber;
                        $_SESSION["MESSAGE_ORDER"] = $id;
